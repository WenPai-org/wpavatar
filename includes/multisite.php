...
class Network {
	 ...
	 */
	private static $default_controlled_options = array(
	...
	public static function init() {
		...
		if ( is_network_admin() ) {
			...
			add_action( 'network_admin_edit_wpavatar_network_settings', array( __CLASS__, 'save_network_settings' ) );
			add_action( 'admin_enqueue_scripts', array( __CLASS__, 'network_admin_scripts' ) );
		}

		// Add network settings link in network plugins list
		add_filter( 'network_admin_plugin_action_links_wpavatar/wpavatar.php', array( __CLASS__, 'add_network_action_links' ) );

		// Add notice in site admin settings when network managed
		if ( is_multisite() && get_site_option( 'wpavatar_network_enabled', 1 ) ) {
			add_action( 'admin_notices', array( __CLASS__, 'network_managed_notice' ) );
			add_action( 'admin_menu', array( __CLASS__, 'maybe_remove_site_menu' ), 999 );
		}

		// Hook for new blog creation
		add_action( 'wpmu_new_blog', array( __CLASS__, 'new_site_created' ) );
	}
...
add_action( 'network_admin_edit_wpavatar_import_site_settings', array( '\WPAvatar\Network', 'import_site_settings' ) );
add_action( 'network_admin_edit_wpavatar_apply_to_all_sites', array( '\WPAvatar\Network', 'apply_to_all_sites' ) );
